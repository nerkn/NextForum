...
export function OneUser({u, className}:{u: userType|null, className?:string}){
    ...
    return (
            <div key={u.id} className={cn("group flex px-2 py-4 min-h-20 rounded-sm hover:animation1 hover:scale-150 hover:bg-[hsl(var(--background-hover))] border  hover:border-[var(--border)] ", className)} style={{transition: "all .2s ease-in-out" }} >
